...
public class secondGreedy {
    ...
    public secondGreedy(String username,String userpwd,String slots)throws SQLException,IOException{
    ...
    public void selectBid(String inputStr, int qid)throws SQLException, IOException{ 
        ...
        String query ="insert into res "
                    ...
                    + "from(select T1.advertiserid, T2.total, T1.key_times, adnum.ad_times, adnum.budget, ad.budget as newbudget "
                         ...
                         + "Order by T2.total*Ad.ctc*T1.key_times/SQRT(adnum.ad_times) desc) "
        ...
        cstmt = conn.createStatement();
        cstmt.execute("Delete from bidTotal R1 where R1.total > (select R2.budget from ad R2 where R2.advertiserid=R1.advertiserid)");
        cstmt.execute("Update bidTotal set rank=rownum");
        cstmt.execute("Update bidTotal set bidTotal.total=(Select MAX(R.total)as total From bidTotal R where R.rank>bidTotal.rank and bidtotal.total<>R.total ) where bidTotal.total<>(Select min(total)as total from bidTotal) and bidTotal.rank<(select count(*) from bidTotal)");
        cstmt.execute("Update res set res.total=(select bidTotal.total from bidTotal where bidtotal.advertiserid=res.advertiserid)");
