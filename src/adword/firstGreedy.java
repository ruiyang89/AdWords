...
public class firstGreedy {
    ...
    public firstGreedy(String username,String userpwd,String slots)throws SQLException,IOException{
    ...
    public void selectKeyword()throws SQLException,IOException{
        ...
        stmt.execute("Create Table Ad (advertiserId INTEGER, budget FLOAT, ctc FLOAT,adCount INTEGER default 0, primary key (advertiserid))");
        ...
        stmt.execute("create table res(advertiserid Integer, total float, key_times Integer, ad_times Integer, budget float, newbudget float, rank Integer default 0, primary key (advertiserid))");
        stmt.execute("create table bidtable(advertiserid integer, bid float)");
        stmt.execute("create table bidTotal(advertiserid integer, total float, rank Integer default 0,primary key (advertiserid))");
        stmt.execute("create or replace type tabletype as table of VARCHAR(255) ");
        stmt.execute("CREATE OR REPLACE FUNCTION split (p_list CLOB, p_sep VARCHAR := ' ') "
